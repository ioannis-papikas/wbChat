...
class thread
{
	...
	public static function add_recipient($threadId, $recipientId)
	{
        ...
        if (empty($threadId) || ($threadId < 0))
            throw new InvalidArgumentException('Invalid thread ID: ' . $threadId);
	...
	{
		if ($description === null)
            throw new InvalidArgumentException('Cannot search for ThreadType with empty description.');
        
        $sqc = new SqlBuilder();
        $sqc->selectTableColumn('threadtype', 'id')->from('threadtype')->where('`description` = "' . $description . '"')->createQuery();
